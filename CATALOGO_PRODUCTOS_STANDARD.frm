...
Private Sub Form_Load()
  ...
  tipoProd = Mid(iNVTARJ01v2_FRM.txtFields(0).Text, 1, 2)
  adoPrimaryRS.Open "select C1_CODIGO Codigo_Producto," & _
                    "C1_TIPO Tipo," & _
                    "C1_DESCRIPCION Descripcion," & _
                    "TO_NUMBER( C1_ESPESOR ) Diametro," & _
                    "TO_NUMBER( C1_LONGITUD ) Longitud," & _
                    "TO_NUMBER( C1_ANCHO ) Ancho," & _
                    "TO_NUMBER( C1_AREA  ) Area," & _
                    "C1_SEPARACION Separacion," & _
                    "TO_NUMBER( C1_ITEMS_ATADO ) Items_x_Atado," & _
                    "TO_NUMBER( C1_PESO ) Peso_Unid," & _
...
Private Sub Form_Load_OLD()
  ...
  If (tipoProd = "AH") Then
      adoPrimaryRS.Open "select C1_CODIGO Codigo_Producto," & _
                    ...
                    "from INV01_DAT " & _
                    "where C1_TIPO = 'AT' " & _
                    "and   C1_CODIGO like '%-H' " & _
                    "Order by C1_TIPO, C1_CODIGO", db, adOpenStatic, adLockOptimistic
  ElseIf (tipoProd = "AT") Then
     adoPrimaryRS.Open "select C1_CODIGO Codigo_Producto," & _
                    "C1_TIPO Tipo," & _
                    "C1_DESCRIPCION Descripcion," & _
                    "TO_NUMBER( C1_ESPESOR ) Diametro," & _
...
End Sub
'* El usuario presiona click en una de las filas.   -- Aquica --
Private Sub grdDataGrid_DblClick()
   Dim tipoProdSF As String
   ...
   End If
   '
   Area = adoPrimaryRS("Area")
   If IsNull(Area) Then
      Area = 0#
   End If
   '
   Separacion = adoPrimaryRS("Separacion")
   If IsNull(Separacion) Then
      Separacion = ""
   End If
   Items_x_Atado = adoPrimaryRS("Items_x_Atado")
   If IsNull(Items_x_Atado) Then
       Items_x_Atado = ""
   End If
   '
   ...
   If IsNull(Diametro) Then
      Diametro = 0#
   End If
   iNVTARJ01v2_FRM.txtCodProducto = Cod_Item
   iNVTARJ01v2_FRM.lblCodBarraArticulo.FontSize = 11
   iNVTARJ01v2_FRM.lblCodBarraArticulo.Font.Name = "IDAutomationHC39M"
   iNVTARJ01v2_FRM.lblCodBarraArticulo = Cod_Item
   If (iNVTARJ01v2_FRM.txtFields(0) = "AT" Or iNVTARJ01v2_FRM.txtFields(0) = "AH") Then
       iNVTARJ01v2_FRM.txtFields(6) = Diametro
   ...
   ElseIf (iNVTARJ01v2_FRM.txtFields(0) = "CE") Then
        ...
        iNVTARJ01v2_FRM.txtFields(7) = Str(Longitud)
        ...
        End If
   ElseIf (Mid(iNVTARJ01v2_FRM.txtFields(0), 1, 2) = "MR") Then
            iNVTARJ01v2_FRM.txtFields(5) = Area
            iNVTARJ01v2_FRM.txtFields(6) = Diametro
            iNVTARJ01v2_FRM.txtFields(7) = Str(Ancho)
            iNVTARJ01v2_FRM.txtFields(8) = Str(Peso)
            iNVTARJ01v2_FRM.txtFields(9) = Str(Longitud)
            iNVTARJ01v2_FRM.txtFields(10) = Separacion
   ElseIf (Mid(iNVTARJ01v2_FRM.txtFields(0), 1, 2) = "SF") Then
         'Tipo de articulo al cual se le realiza servicio de fabricacion (SF):
         tipoProdSF = Mid(Cod_Item, (Len(Cod_Item) - 1), 2)
         Select Case tipoProdSF
            Case "MR", "AT"
                iNVTARJ01v2_FRM.txtFields(5) = Area
                iNVTARJ01v2_FRM.txtFields(6) = Diametro
                iNVTARJ01v2_FRM.txtFields(7) = Str(Ancho)
            ...
            Case "CE"
                 ...
                 iNVTARJ01v2_FRM.txtFields(9) = Str(Ancho)
                 'MsgBox Val(InStr(Cod_Item, "C"))
                 If (InStr(Cod_Item, "C") = (Len(RTrim(Cod_Item)) - 3)) Then
                    iNVTARJ01v2_FRM.Combo_TipoCercha.Text = "CONTINUAS"
                    'iNVTARJ01v2_FRM.txtFields(11).Text = 45
                 Else
                    iNVTARJ01v2_FRM.Combo_TipoCercha.Text = "DISCONTINUAS"
            ...
            End Select
   End If
   iNVTARJ01v2_FRM.txtFields(11).Text = Items_x_Atado
   '**Fin:
   Unload Me
End Sub
